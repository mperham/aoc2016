...
WIDTH = 50
HEIGHT = 6
SCREEN = Array.new(6) { Array.new(WIDTH) { nil } }
...
def rotate_row(y, amt)
  arr = SCREEN[y]
  SCREEN[y] = arr[-amt..-1] + arr[0...(WIDTH-amt)]
  amt
end
...
input.each_line do |line|
  case line
  when /rect (\d+)x(\d+)/
    rect(Integer($1), Integer($2))
  when /rotate row y=(\d+) by (\d+)/
    rotate_row(Integer($1), Integer($2))
  when /rotate column x=(\d+) by (\d+)/
    rotate_column(Integer($1), Integer($2))
  else
...
count = 0
SCREEN.each { |row| row.each { |col| count += 1 if col } }
puts "#{count} pixels set after #{cmds} commands"
