...
input.each_line do |line|
  ...
  while true
    if line.start_with?("[")
      parts = line.match(/\[(\w+)\](.*)/)
      bad, line = parts[1], parts[2]
      #p [:bad, bad, line, abba?(bad)]
      break has_good_abba = false if abba?(bad)
    else
      parts = line.match(/(\w+)(\[.*)?/)
      good, line = parts[1], parts[2]
      #p [:good, good, line, abba?(good)]
      has_good_abba ||= abba?(good)
    end
    break if line == nil
  end
  ok << original if has_good_abba
end
