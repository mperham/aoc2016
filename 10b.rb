...
class Bot
  ...
  def instruction(low, lowidx, high, highidx)
    @low = low
    @lowidx = lowidx
    @high = high
    @highidx = highidx
  ...
  def distribute
    raise unless @val1 && @val2
    puts @idx if [@val1, @val2].include?(61) && [@val1, @val2].include?(17)

    if @val1 < @val2
      @low[@lowidx].give(@val1)
      @high[@highidx].give(@val2)
    else
      @low[@lowidx].give(@val2)
      @high[@highidx].give(@val1)
    end
    @val1 = nil
    @val2 = nil
  end
...
def output(idx)
  OUTPUT[idx] ||= Output.new
  OUTPUT
end

def bot(idx)
  BOTS[idx] ||= Bot.new(idx)
  BOTS
end
...
input.each_line do |line|
  ...
  when /bot (\d+) gives low to (\w+) (\d+) and high to (\w+) (\d+)/
    ...
    t1id = Integer($3)
    t2id = Integer($5)
    target1 = $2 == 'output' ? output(t1id) : bot(t1id)
    target2 = $4 == 'output' ? output(t2id) : bot(t2id)
    bot = BOTS[bid] ||= Bot.new(bid)
    bot.instruction(target1, t1id, target2, t2id)
...
puts OUTPUT[0].value * OUTPUT[1].value * OUTPUT[2].value
