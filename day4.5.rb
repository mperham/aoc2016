...
def valid?(room, checksum)
  counts = {}
  26.times {|idx| counts[('a'.ord + idx).chr] = 0 }

  room.each_char do |char|
    counts[char] += 1 if counts[char]
  end

  sorted = counts.sort do |a, b|
    if b[1] != a[1]
      b[1] - a[1]
    else
      a[0].ord - b[0].ord
...
def decrypt(room, sector)
  ...
  room.each_char do |char|
    (next dec << ' ') if char == '-'
    dec << (97 + (((char.ord + mod) - 97) % 26)).chr
  end

  dec.join + sector.to_s
end

input.each_line do |line|
  match = line.match(/(.*)-(\d+)\[([a-z]{5})\]/)
  roomname = match[1]
  sector = match[2].to_i
  checksum = match[3]
  if valid?(roomname, checksum)
    room = decrypt(roomname, sector)
    puts room if room !~ /egg|basket|plastic|hunt|dye|flower|candy|jelly|bunny|rabbit|chocolate/
  end
end
